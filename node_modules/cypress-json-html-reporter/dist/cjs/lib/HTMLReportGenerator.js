...
class HTMLReportGenerator {
  ...
  async convertJSONFolderToHTML(folderPath) {
    ...
    try {
      ...
      for (const file of files) {
        if (file.endsWith('.json') && file !== 'test-report.json') {
          ...
          jsonFilesToProcess.push(filePath);
        }
      }
      ...
      const consolidatedFilePath = path.join(folderPath, 'test-report.json');
      ...
      // Read the video map written by cypress.config.js after:spec handler
      const VIDEO_MAP_FILE = path.join(folderPath, '.video-map.json');
      if (fs.existsSync(VIDEO_MAP_FILE)) {
        try {
          this.videoMap = JSON.parse(fs.readFileSync(VIDEO_MAP_FILE, 'utf8'));
        } catch (e) {
          this.videoMap = {};
        }
      } else {
      ...
      // Clean up individual JSON files after they were merged
      for (const filePath of jsonFilesToProcess) {
        try {
          fs.unlinkSync(filePath);
        } catch (unlinkErr) {
          console.warn(`Failed to delete temporary JSON file: ${filePath}`, unlinkErr);
        }
      }
    } catch (error) {
  ...
  generateHTMLReport(historyData) {
    ...
    // Process test results to embed screenshots and videos as data URLs.
    ...
    const processedTestResults = this.testResults.map(test => {
      if (test.screenshot && !test.screenshot.startsWith('data:')) {
        ...
        if (dataUrl) test.screenshot = dataUrl;
      }
      // For FAILED tests: embed video as base64 data URL if available
      if (test.status === 'FAILED' && !test.video) {
        const videoPath = videoMap[test.suiteName];
        if (videoPath) {
          test.video = this.embedVideo(videoPath);
        }
      }
      return test;
    });
...
<body>
    ...
    <div id="Dashboard" class="tabcontent" style="display: block;">
      ...
      <div class="cards-group">
        ...
        <div class="cards">
          ...
          <div class="card" onclick="filterByStatus('SKIPPED')" title="View Skipped Tests">
            <h3>Skipped</h3>
            <p>${summary.skipped}</p>
          </div>
        </div>
  ...
  <script>
    ...
    function renderTable(page) {
      ...
      currentData.forEach(test => {
        ...
        let videoHTML = 'No Video';
        if (test.status === 'FAILED' && test.video && test.video.startsWith('data:')) {
          videoHTML = '<video class="video-thumb" controls preload="none"><source src="' + test.video + '" type="video/mp4"></video>';
        }
        html += \`<tr data-suite="\${escapeHtml(test.suiteName || '')}" data-test-name="\${escapeHtml(test.testName || '')}" data-status="\${test.status || ''}" data-error="\${escapeHtml(test.errors ? test.errors.map(e => e.message).join(' | ') : '')}">
          ...
          <td class="\${test.status === 'PASSED' ? 'passed' : test.status === 'FAILED' ? 'failed' : test.status === 'SKIPPED' ? 'skipped' : ''}">\${test.status || ''}</td>
          <td class="error-cell">
    ...
    function exportTableToExcel(tableID, filename = '') {
      ...
      var style = '<style>' +
        ...
        'td.skipped { background-color: #fff3cd; color: #856404; }' +
        '</style>';
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        if (test.status === 'PASSED') {
          ...
          statusCell = '<td class="skipped">' + (test.status || '') + '</td>';
        } else {
  ...
  /**
   * Converts a video file to a base64 data URL.
  ...
  embedVideo(videoPath) {
    try {
      if (fs.existsSync(videoPath)) {
        ...
        return `data:${mimeType};base64,${videoData.toString('base64')}`;
      }
      return null;
    } catch (e) {
      return null;
